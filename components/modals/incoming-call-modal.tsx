...
const IncomingCallModal = () => {
	...
	const handleCallAccepted = () => {
		...
		axios
			...
			.then((res) => {
				console.log(
					"🚀 ~ file: incoming-call-modal.tsx:53 ~ .then ~ res:",
					res,
				);
			})
			.catch((err) => {
				console.log(
					"🚀 ~ file: incoming-call-modal.tsx:58 ~ handleCallAccepted ~ err:",
					err,
				);
				...
				<DialogContent className="bg-white dark:bg-[#313338] text-black dark:text-white p-0 items-center">
					<DialogHeader className="pt-6 px-6">
						<Avatar className="mx-auto mt-6 h-20 w-20">
							<AvatarImage src={caller?.profile?.imageUrl} alt="avatar" />
						</Avatar>
						<DialogTitle className="text-2xl font-bold text-center">
							Incoming Call
						</DialogTitle>
						<DialogDescription className="text-center text-gray-600 dark:text-gray-400">
							You have an incoming call from {caller?.profile?.name}
						</DialogDescription>
					</DialogHeader>
